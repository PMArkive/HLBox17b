...
namespace HLBox17b.Classes.Tools
    ...
    class ShellAPI
    {
		...
		public const Int32 FILE_ATTRIBUTE_DIRECTORY = 0x10;
		...
		public static string GetFileType(string szExt)
			{
			...
			var res = ShellAPI.SHGetFileInfo(szExt, ShellAPI.FILE_ATTRIBUTE_NORMAL, out shInfo, (uint)Marshal.SizeOf(shInfo), dwAttribs);
			if (res == IntPtr.Zero)
				throw Marshal.GetExceptionForHR(Marshal.GetHRForLastWin32Error());
			return shInfo.szTypeName;
			}

		public static string GetFolderType(string FolderSample)
			{
			ShellAPI.SHFILEINFO shInfo = new ShellAPI.SHFILEINFO();
			ShellAPI.SHGFI dwAttribs =
				ShellAPI.SHGFI.SHGFI_USEFILEATTRIBUTES |
				ShellAPI.SHGFI.SHGFI_TYPENAME;
			var res = ShellAPI.SHGetFileInfo(FolderSample, ShellAPI.FILE_ATTRIBUTE_DIRECTORY, out shInfo, (uint)Marshal.SizeOf(shInfo), dwAttribs);
			if (res == IntPtr.Zero)
				throw Marshal.GetExceptionForHR(Marshal.GetHRForLastWin32Error());
			return shInfo.szTypeName;
			}

		public static Icon GetSmallIcon(string szExt)
			{
			ShellAPI.SHFILEINFO shInfo = new ShellAPI.SHFILEINFO();
			ShellAPI.SHGFI dwAttribs =
			...
			ShellAPI.DestroyIcon(shInfo.hIcon);        // Cleanup    
			return icon;
			}

		public static Icon GetFolderIcon(string FolderSample)
			{
			ShellAPI.SHFILEINFO shInfo = new ShellAPI.SHFILEINFO();
			ShellAPI.SHGFI dwAttribs =
				ShellAPI.SHGFI.SHGFI_ICON|
		...
		public static int GetFolderIconIndex(string FolderSample)
			{
			ShellAPI.SHFILEINFO shInfo = new ShellAPI.SHFILEINFO();
			ShellAPI.SHGFI dwAttribs =
				ShellAPI.SHGFI.SHGFI_ICON |
		...
		public static ShellAPI.SPECINFO GetDesktopFolderInfos()
			{
			...
			SPECINFO spcInf = new ShellAPI.SPECINFO();
			spcInf.iIcon = shInfo.iIcon;
			spcInf.szDisplayName = shInfo.szDisplayName;
			spcInf.szPath = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
			return spcInf;
			}
